...
- **协议转换**: 内置 OpenAI ↔ Qwen ↔ Anthropic ↔ Responses 协议转换器
- **Responses API 支持**: 内置 `/openai/responses` 端点，自动转换为 Chat Completions 格式
...
# 或 (Responses API)
http://localhost:5564/openai/responses
```
...
| `openai-to-qwen` | 将 OpenAI Chat Completions 格式转换为本地 Qwen CLI 格式，添加请求验证头并使用本地密钥访问 Qwen Code 站点 |
| `anthropic-to-qwen` | 将 Anthropic Messages 格式转换为本地 Qwen CLI 格式，添加请求验证头并使用本地密钥访问 Qwen Code 站点 |
| `anthropic-to-openai` | 将 Anthropic Messages 请求格式转换为 OpenAI Chat Completions 格式，并将上游响应转回 Anthropic 格式 |
| `responses-to-chat` | 将 OpenAI Responses API 请求转换为 Chat Completions 格式，并将上游 Chat 响应转回 Responses 格式 |
...
│                    Bifrost Server                           │
│  ┌─────────────┐ ┌──────────────┐ ┌─────────────┐ ┌───────┐ │
│  │ /openai/... │ │/openai/resp..│ │/anthropic/..│ │Adapter│ │
│  └──────┬──────┘ └──────┬───────┘ └──────┬──────┘ └───┬───┘ │
│         │               │                │            │     │
│         └───────────────┴────────────────┴────────────┘     │
│                          │                                  │
